...
void Print()
{
    ...
    Array.Reverse(_arr);
    Console.WriteLine("Вiдсортованi за спаданням значення масиву:");
    Console.WriteLine("Мiнiмальне значення масиву: {0:0.0000}", _arr[_arr.GetUpperBound(0)]);
    Console.WriteLine("Максимальне значення масиву: {0:0.0000}", _arr[_arr.GetLowerBound(0)]);
}
...
double Function(double x)
{
    if (!double.IsNaN(x))
    {
        ...
        double x2 = 0.5 * x;
        x = (x1 + 2 * x2 + 9) / 0.666;
        return ((3 * Math.Cos(x) + Math.Cos(3 * x)) / 4) + Math.Pow(x2, 7);
    }
    return 0;
}
...
void R(double[] arr)
{
    ...
    Console.WriteLine("Кількість елементів масиву, менших за середнє значення: {0:0}", caunt);
}
